...
const DashboardStreamComponent = (
  ...
  ref: React.Ref<HTMLDivElement>
    ...
    const clusters = useMemo(() => {
        const dayFilter = (d: string) => isSameDay(new Date(d), selectedDate);
        
        const allItems: StreamItem[] = [
            ...memos.filter(m => dayFilter(m.timestamp)).map(m => ({ ...m, itemType: 'memo' as const })),
            ...bookmarks.filter(b => dayFilter(b.timestamp)).map(b => ({ ...b, itemType: 'bookmark' as const })),
            ...history.filter(h => dayFilter(h.visit_time)).map(h => ({ ...h, timestamp: h.visit_time, itemType: 'history' as const }))
        ];
